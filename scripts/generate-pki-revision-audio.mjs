...
const TMP_DIR = path.join(ROOT, "tmp-audio", "pki");
const AUDIO_DIR = path.join(ROOT, "public", "audio", "pki");
const REVISION_DIR = path.join(ROOT, "public", "data", "pki", "revision");
const INDEX_FILE = path.join(REVISION_DIR, "index.json");
...
function cleanForSpeech(value) {
  return String(value || "")
    .replaceAll("â†’", " to ")
    .replaceAll("â†“", " then ")
    .replaceAll("â†”", " mutual trust with ")
    .replaceAll("Ã—", " times ")
    .replace(/[→⇒]/g, " to ")
    .replace(/[↓]/g, " then ")
...
function buildNarration(phase) {
  const lines = [
    ...
    `This PKI revision phase contains ${phase.topics.length} exam topics.`
  ];
...
async function synthesizePhase(piperExe, phaseInfo) {
  ...
  const basename = path.basename(phaseInfo.audio, ".mp3");
  const narrationFile = path.join(TMP_DIR, `${basename}.txt`);
  const wavFile = path.join(TMP_DIR, `${basename}.wav`);
  const mp3File = path.join(AUDIO_DIR, `${basename}.mp3`);

  await fs.mkdir(TMP_DIR, { recursive: true });
  await fs.mkdir(AUDIO_DIR, { recursive: true });
  await fs.writeFile(narrationFile, buildNarration(phase), "utf8");

  console.log(`Synthesizing ${basename}.wav`);
...
async function main() {
  ...
  for (const phase of index.phases) {
    phase.audio = `/audio/pki/${path.basename(phase.audio, path.extname(phase.audio))}.mp3`;
    await synthesizePhase(piperExe, phase);
  }

  await fs.writeFile(INDEX_FILE, `${JSON.stringify(index, null, 2)}\n`, "utf8");
  console.log("PKI revision audio generated.");
}
